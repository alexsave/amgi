...
export const stopRecording = async () => {
  try {
    ...
    const audio_blob = await recorder.stopRecording();
    console.log('Recording stopped, blob details:', {
      size: audio_blob.size,
      type: audio_blob.type
    });
    
    if (audio_blob.size === 0) {
      throw new Error('No audio data recorded');
    }

    // Verify that we have a valid MP3 blob
    if (audio_blob.type !== 'audio/mpeg' && audio_blob.type !== 'audio/mp3') {
      console.warn('Warning: Audio blob is not MP3 format:', audio_blob.type);
    }
